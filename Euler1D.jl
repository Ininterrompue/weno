...
function euler(; γ=7/5, cfl=0.3, t_max=1.0)
    ...
    while t < t_max
        ...
        #     F̂.ρ[i]  = Weno.update_numerical_flux(q.ρ,  f.ρ,  wepar)
        #     F̂.ρu[i] = Weno.update_numerical_flux(q.ρu, f.ρu, wepar)
        #     F̂.E[i]  = Weno.update_numerical_flux(q.E,  f.E,  wepar)
        # end
        ...
        for i in gridx.cr_cell
            ...
            Ĝ.ρ[i]  = Weno.update_numerical_flux(q.ρ,  f.ρ,  wepar)
            Ĝ.ρu[i] = Weno.update_numerical_flux(q.ρu, f.ρu, wepar)
            Ĝ.E[i]  = Weno.update_numerical_flux(q.E,  f.E,  wepar)
