...
function update_numerical_fluxes!(i, F̂, q, f, wepar, ada)
    F̂.ρ[i]  = Weno.update_numerical_flux(q.ρ,  f.ρ,  wepar, ada)
    F̂.ρu[i] = Weno.update_numerical_flux(q.ρu, f.ρu, wepar, ada)
    F̂.E[i]  = Weno.update_numerical_flux(q.E,  f.E,  wepar, ada)
end
...
function euler(; γ=7/5, cfl=0.3, t_max=1.0)
    ...
    while t < t_max
        ...
        #     update_numerical_fluxes!(i, flux.F_hat, q, f, wepar, false)
        # end
    ...
    @printf("%d iterations. t_max = %2.3f. Elapsed time = %3.3f\n", 
        counter, t, time() - t0)
    plot_system(state.Q, gridx, "lax_512_ada")
end

# BenchmarkTools.@benchmark euler(t_max=0.13);
